// db-sync.js v14 yüklendi
console.log('[db-sync] modül yükleniyor… UA=', navigator.userAgent.slice(0, 80));
...
// =====================================================================
//  Mobil redirect dönüşünü işle (sayfa yüklendiğinde bir kez)
// =====================================================================
async function processMobileRedirectResult() {
    let result = null;
    try {
        result = await getRedirectResult(auth);
    } catch (e) {
        const action = window.localStorage.getItem(REDIRECT_PENDING_KEY);
        const fallback = window.localStorage.getItem(REDIRECT_FALLBACK_KEY);
        if (e.code === 'auth/credential-already-in-use' && action === 'link' && !fallback) {
            // Otomatik fallback: çıkış + Google ile direkt giriş (yine redirect)
            console.log('[db-sync] Google hesabı zaten kayıtlı, otomatik geri yükleme...');
            try {
                window.localStorage.setItem(REDIRECT_FALLBACK_KEY, '1');
                _skipAutoAnon = true;
                ...
                return; // sayfa yine redirect olur
            } catch (e2) {
                console.warn('[db-sync] redirect fallback hatası:', e2);
                _skipAutoAnon = false;
                window.localStorage.removeItem(REDIRECT_PENDING_KEY);
                window.localStorage.removeItem(REDIRECT_FALLBACK_KEY);
            }
        } else {
            console.warn('[db-sync] redirect result hata:', e);
            window.localStorage.removeItem(REDIRECT_PENDING_KEY);
            window.localStorage.removeItem(REDIRECT_FALLBACK_KEY);
        }
        return;
    ...
    if (result) {
        ...
        window.localStorage.removeItem(REDIRECT_PENDING_KEY);
        window.localStorage.removeItem(REDIRECT_FALLBACK_KEY);
        console.log('[db-sync] redirect login OK:', result.user.email);
    }
}
...
console.log('[db-sync] modül yüklendi ✓');
window.__dbSyncLoading = false;
window.__dbSyncReady = true;
...
const _isMobileLike = (() => {
    try {
        ...
        return /iPad|iPhone|iPod|Android|webOS|BlackBerry|Windows Phone/i
            .test(navigator.userAgent || '');
    } catch (_) { return false; }
})();
const REDIRECT_PENDING_KEY = '_dbsync_redirect_pending';
const REDIRECT_FALLBACK_KEY = '_dbsync_redirect_fallback';
...
window.dbAuth = {
    ...
    /** Token süresi dolduysa Google ile yeniden popup/redirect açıp tazele */
     ...
     * Token varsa döner, yoksa popup/redirect ile yeniden alır.
     */
    async ensureGoogleToken() {
        ...
        if (_isMobileLike) {
            // Mobilde sayfa redirect olacağından kullanıcıyı yönlendir
            throw new Error(
                'Drive yetkisi süresi dolmuş. Veri Yönetimi → "Google Hesabını Tazele" ' +
                'butonuyla yenilemeniz gerekiyor.'
            );
        }
        // Desktop: popup ile sessiz tazele
        const provider = new GoogleAuthProvider();
        provider.addScope('https://www.googleapis.com/auth/drive.file');
        const result = await signInWithPopup(auth, provider);
        const credential = GoogleAuthProvider.credentialFromResult(result);
    ...
    /** Mobilde "yön. olunuyor mu?" göstergesi için */
    isMobileFlow() { return _isMobileLike; }
};
