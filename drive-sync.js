...
async function autoInit() {
    ...
    // GIS prompt:'none' bazı durumlarda callback'i hiç tetiklemez (oturum yok / 3rd-party cookie kısıtı).
    // Bu durumda nokta gri kalmasın diye 6sn'lik koruma timeout'u koyuyoruz.
    const SILENT_TIMEOUT_MS = 6000;
    function withTimeout(promise, ms) {
        return new Promise((resolve, reject) => {
            const t = setTimeout(() => reject(new Error('silent-timeout')), ms);
    ...
    try {
        await withTimeout(requestToken({ interactive: false }), SILENT_TIMEOUT_MS);
        ...
        // Sessiz giriş başarısız ya da hiç dönmedi → kullanıcının manuel girişine kalır.
        // Yine de mevcut local veri/folder varsa "warn" gösterip yaşamı sürdürelim.
        if (window.localStorage.getItem(LS_FOLDER_ID)) {
            setStatus('warn', 'Giriş gerekli (klasör hazır)');
        } else {
