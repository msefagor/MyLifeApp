const CACHE_NAME = 'glass-planner-v26';
const urlsToCache = [
  './',
  './index.html',
  './namaz.html',
...
self.addEventListener('fetch', event => {
  ...
  // devam.html (Yoklama) ve namaz.html (Namaz Takibi) için her zaman taze ağ isteği
  // — sürüm takılmasını önler (yeni sekmeler/özellikler hemen gelsin)
  if (url.pathname.endsWith('/devam.html') || url.pathname.endsWith('devam.html') ||
      url.pathname.endsWith('/namaz.html') || url.pathname.endsWith('namaz.html')) {
    event.respondWith(
  ...
  // Ses dosyaları (okuma modu müziği) — tarayıcı Range istekleriyle parça parça
  // indirdiği için SW araya girmemeli; yoksa çalma/ileri sarma bozulur.
